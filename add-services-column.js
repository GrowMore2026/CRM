...
async function runMigration() {
  console.log('Running migration: adding service column to clients table...');

  // Attempt using an RPC function if it exists
  const { error } = await supabase.rpc('exec_sql', {
    sql: `ALTER TABLE clients ADD COLUMN IF NOT EXISTS "service" jsonb DEFAULT '[]'::jsonb;`
  });
  ...
  if (error) {
    ...
    if (testError && testError.code === 'PGRST204') {
      console.log('❌ Column "service" does NOT exist in clients table.');
      console.log('\n✅ Please run this SQL in your Supabase dashboard → SQL Editor:\n');
      console.log('ALTER TABLE clients ADD COLUMN IF NOT EXISTS "service" jsonb DEFAULT \'[]\'::jsonb;');
      console.log('notify pgrst, \'reload schema\';');
    } else if (testError) {
      console.log('❌ Error:', testError.message);
      console.log('Please add the column manually in Supabase SQL editor: ALTER TABLE clients ADD COLUMN "service" jsonb DEFAULT \'[]\'::jsonb;');
    } else {
      console.log('✅ Column "service" already exists! Data:', testData);
    }
  } else {
