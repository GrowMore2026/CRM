...
async function runTest() {
  ...
  const testTask = {
    ...
    description: 'Testing task insert after SQL patch',
    assignedTo: '001', 
    createdBy: 'GrowMore Admin',
    status: 'Pending'
  };

  const { data: taskData, error: taskError } = await supabase.from('tasks').insert([testTask]).select();
  if (taskError) {
    console.error('Task Insert Error:', JSON.stringify(taskError, null, 2));
  } else {
